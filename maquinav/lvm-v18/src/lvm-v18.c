...
void imprimeStr(unsigned char *buffer, int inicioStr);
			...
			while (indice < size) {
				...
				if (buffer[indice] == 0x01) {
					...
					if (buffer[indice] == 0x03) {
						...
						indiceMemoria = retornaInt(buffer, indice);
						indice += 4;
						imprimeStr(buffer, inicioDados + indiceMemoria);
					}
					...
					if (buffer[indice] == 0x04) {
						...
						insereMem(entradaInt, buffer, inicioDados + indiceMemoria);
...
void imprimeStr(unsigned char *buffer, int inicioStr) {
	...
	while (buffer[inicioStr] != '\0') {
		printf("%c", buffer[inicioStr++]);
	}
}
